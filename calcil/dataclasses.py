# Description:
#  
# Written by Ruiming Cao on March 26, 2022
...
if '_DATACLASS_INSTANCES' not in globals():
    _DATACLASS_INSTANCES = WeakSet()

def register_dataclass_instance(ins):
    _DATACLASS_INSTANCES.add(ins)

def get_dataclass_instances(serialize=False):
    if serialize:
        return [(serialization.to_state_dict(ins), ins.__class__) for ins in _DATACLASS_INSTANCES]
    else:
        return [ins for ins in _DATACLASS_INSTANCES]

def reset_dataclass_instances():
    global _DATACLASS_INSTANCES
    _DATACLASS_INSTANCES = WeakSet()


# below is primarily from flax.struct.dataclass (adding __post_init__)
#
...
def dataclass(clz: _T) -> _T:
  ...
  def __post_init__(self):
      ...
      register_dataclass_instance(self)
  clz.__post_init__ = __post_init__
