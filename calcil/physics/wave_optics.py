...
def genPupil(shape, pixel_size, NA, wavelength, NA_in=0.0, fx_illu=0.0, fy_illu=0.0):
    ...
    pupil = np.asarray(
        (fxlin[np.newaxis, :] - fx_illu) ** 2 + (fylin[:, np.newaxis] - fy_illu) ** 2 <= pupil_radius ** 2)
    if NA_in != 0.0:
        pupil[(fxlin[np.newaxis, :] - fx_illu) ** 2 + (fylin[:, np.newaxis] - fy_illu) ** 2 < (
                    NA_in / wavelength) ** 2] = 0.0
...
def propKernel(dim_yx, prop_distances, pixel_size, wavelength, RI):
    ...
    prop_kernel = jnp.exp(1.0j * 2.0 * jnp.pi * jnp.abs(prop_distances[:, jnp.newaxis, jnp.newaxis]) * (
            (RI / wavelength) ** 2 - fxlin[jnp.newaxis, jnp.newaxis, :] ** 2 - fylin[jnp.newaxis, :,
                                                                               jnp.newaxis] ** 2) ** 0.5)
