...
This module provides functions to perform gradient descent-based reconstruction with any given differentiable forward
model. The module supports single variable reconstruction and multi-variable reconstruction with different optimization
settings for each variable. The module also supports checkpoint saving and tensorboard logging.
"""
...
class ReconVarParameters:
    ...
    Args:
        ...
        opt (Union[str, optax.GradientTransformation]): optimizer. Either a string to specify the optimizer in optax or
            an optax.GradientTransformation object.
        opt_kwargs (dict): a dictionary of keyword arguments for the optimizer.
        schedule (Union[str, optax.Schedule]): learning rate schedule. Either a string to specify the schedule in optax
            or an optax.Schedule object.
        schedule_kwargs (dict): a dictionary of keyword arguments for the learning rate schedule.
        delay_update_n_iter (int): number of iterations to delay the update of the variable. The optimization will do
            nothing for the first n iterations. Note that this goes with iteration (step), not epoch.
        update_every (int): update the variable every n iterations. Default is 1 (update every iteration). When set to
            n, the gradient will be accumulated for n iterations before updating the variable. See: optax.apply_every.
            Note that this goes with iteration (step), not epoch.
    """
    lr: float = 0
...
def update_iter_sgd(state: train_state.TrainState,
                    input_dict, rngs, loss_fn):
...
def _set_up_tx(var_params: ReconVarParameters):
    ...
    When var_params.lr <= 0, the optimizer is set to optax.set_to_zero() (no update). When var_params.delay_update_n_iter
    > 0, the optimizer is delayed update for the first n iterations (note that this goes with iteration, not epoch).
...
def reconstruct_sgd(forward_fn: Callable,
    ...
    Args:
        ...
        output_fn (callable): output function that renders output images. output_fn takes the reconstruction variables
            and train state (from which forward_fn can be called with the variables) as input and returns a dictionary
            of output images.
    ...
    Returns:
        dict: final reconstructed variables at the end of the reconstruction.
        dict: dictionary of output images generated by output_fn at different iterations.
...
def reconstruct_multivars_sgd(forward_fn: Callable,
    ...
    Args:
        ...
        var_params_pytree (dict): nested dictionary of optimization parameters for each variable. all variables need to
            be included in the dictionary. Set the learning rate to 0 to freeze the variable. The keys of the dictionary
            should correspond to the keys of the variables dictionary. Each entry is a ReconVarParameters object
            (see calcil.reconstruction.ReconVarParameters).
        data_loader (generator): generator that yields input dictionary (see calcil.data_utils.loader_from_numpy)
        loss (Loss): loss function (see calcil.loss)
        recon_param (ReconIterParameters): reconstruction settings (see calcil.reconstruction.ReconIterParameters)
        output_fn (callable): output function that renders output images. output_fn takes the reconstruction variables
...
def load_checkpoint_and_output(load_path, output_fn=None):
    ...
    Args:
        ...
        output_fn (callable): output function that renders output images. output_fn takes the reconstruction variables
            as input and returns a dictionary of output images.
    Returns:
        dict: reconstructed variables in the checkpoint.
